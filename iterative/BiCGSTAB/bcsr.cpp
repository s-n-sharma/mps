...
class bcsr_tensor {
    public:
        ...
        MTL::ComputePipelineState* pso_upomega_upxr_laa;
        MTL::ComputePipelineState* pso_dip;
        MTL::ComputePipelineState* pso_upalpha_ups;
        MTL::ComputePipelineState* pso_upp;

        uint32_t logical_rows;      // Original matrix size (e.g. 10)
        uint32_t logical_cols;
        uint32_t num_rows;       // Round up to nearest 4 (e.g. 12)
        uint32_t num_cols; 
        uint32_t num_block_rows;    // padded_rows / 4 (e.g. 3)
        uint32_t num_vals;
        ...
        void loadPipelines() {
            ...
            NS::String* libraryPath = NS::String::string("./bcsr_spmv.metallib", NS::UTF8StringEncoding);
            MTL::Library* library = device->newLibrary(libraryPath, &error);
            
            if (!library) {
                std::cerr << "Failed to load library: " << "spmv" << error->localizedDescription()->utf8String() << std::endl;
                return;
            }
            ...
            if (!library) {
                std::cerr << "Failed to load library: " << "coo csr " << error->localizedDescription()->utf8String() << std::endl;
                return;
            }
            ...
            libraryPath = NS::String::string("./cgd.metallib", NS::UTF8StringEncoding);
            library = device->newLibrary(libraryPath, &error);

            if (!library) {
                std::cerr << "Failed to load library: " <<  " cgd " << error->localizedDescription()->utf8String() << std::endl;
                return;
            }
            ...
            libraryPath = NS::String::string("./bicgstab.metallib", NS::UTF8StringEncoding);
            library = device->newLibrary(libraryPath, &error);

            if (!library) {
                std::cerr << "Failed to load library: " << "bicgstab" << error->localizedDescription()->utf8String() << std::endl;
                return;
            }
        ...
        bcsr_tensor( 
            ...
            this->logical_rows = num_rows;
            this->logical_cols = num_cols;
            
            this->num_block_rows = (num_rows + block_size - 1) / block_size;
            uint32_t num_block_cols = (num_cols + block_size - 1) / block_size;
            
            this->num_rows = this->num_block_rows * block_size;
            this->num_cols = num_block_cols * block_size;

            
            float* vals_ptr = (float*)values.data_ptr<float>();
            uint32_t* rptr_ptr = (uint32_t*)row_ptr.data_ptr<int32_t>();
            uint32_t* cind_ptr = (uint32_t*)col_ind.data_ptr<int32_t>();

            uint32_t vals_bytes = values.size(0) * sizeof(float);
            uint32_t rptr_bytes = row_ptr.size(0) * sizeof(uint32_t);
            uint32_t cind_bytes = col_ind.size(0) * sizeof(uint32_t);

            this->vals = device->newBuffer(vals_bytes, MTL::ResourceStorageModePrivate);
            this->row_ptr = device->newBuffer(rptr_bytes, MTL::ResourceStorageModePrivate);
            this->col_ind = device->newBuffer(cind_bytes, MTL::ResourceStorageModePrivate);

            MTL::Buffer* stage_vals = device->newBuffer(vals_ptr, vals_bytes, MTL::ResourceStorageModeShared);
            MTL::Buffer* stage_rptr = device->newBuffer(rptr_ptr, rptr_bytes, MTL::ResourceStorageModeShared);
            MTL::Buffer* stage_cind = device->newBuffer(cind_ptr, cind_bytes, MTL::ResourceStorageModeShared);

            MTL::CommandBuffer* cmd = queue->commandBuffer();
            MTL::BlitCommandEncoder* blit = cmd->blitCommandEncoder();
            blit->copyFromBuffer(stage_vals, 0, this->vals, 0, vals_bytes);
            blit->copyFromBuffer(stage_rptr, 0, this->row_ptr, 0, rptr_bytes);
            blit->copyFromBuffer(stage_cind, 0, this->col_ind, 0, cind_bytes);
            blit->endEncoding();
            cmd->commit();
            cmd->waitUntilCompleted();
            ...
            this->buf_x = device->newBuffer(this->num_cols * sizeof(float), MTL::ResourceStorageModePrivate);
            this->buf_b = device->newBuffer(this->num_rows * sizeof(float), MTL::ResourceStorageModePrivate);
            
            cmd = queue->commandBuffer();
            blit = cmd->blitCommandEncoder();

            blit->fillBuffer(this->buf_x, NS::Range::Make(0, this->num_cols * sizeof(float)), 0);
            blit->fillBuffer(this->buf_b, NS::Range::Make(0, this->num_rows * sizeof(float)), 0);
        ...
        void mv_buffer(
            ...
            enc->setBuffer(x, 0, 3);
            enc->setBuffer(b, 0, 4);
            enc->setBytes(&this->num_block_rows, sizeof(uint32_t), 5);
            enc->dispatchThreads( MTL::Size::Make(this->num_block_rows, 1, 1), MTL::Size::Make(THREADS_PER_GROUP, 1, 1));
        }
        ...
        void spvmv(
            ...
            enc->setBytes(&this->num_block_rows, sizeof(uint32_t), 7);
        ...
        void cgd_op(
            ...
            memcpy(x_vals, stage_x->contents(), (this->logical_rows) * sizeof(float));
        ...
        void bicgstab_op(
            ...
            MTL::Buffer* stage_r_norm = this->device->newBuffer(sizeof(float), MTL::ResourceStorageModeShared);
            float* stage_r_norm_ptr = (float*)stage_r_norm->contents();
            stage_r_norm_ptr[0] = 1.0e9f;
            ...
            stage_b->release();
            stage_scalars->release();

            MTL::CommandBuffer* cmd_pre = queue->commandBuffer();
            MTL::ComputeCommandEncoder* enc_pre = cmd_pre->computeCommandEncoder();
            
            enc_pre->setComputePipelineState(this->pso_iprd);
            enc_pre->setBuffer(r, 0, 0);
            enc_pre->setBuffer(r, 0, 1);
            enc_pre->setBuffer(scalars, 1 * sizeof(float), 2);
            enc_pre->setBytes(&this->num_rows, sizeof(uint), 3);
            enc_pre->dispatchThreads(MTL::Size::Make(this->num_rows, 1, 1), MTL::Size::Make(THREADS_PER_GROUP, 1, 1));
            enc_pre->endEncoding();
            cmd_pre->commit();
            cmd_pre->waitUntilCompleted();
            ...
            for (int i = 0; i < iter_number; i++) {
                ...
                //upp: p,r,v,scalars & size

                
                enc->setComputePipelineState(this->pso_upp);
                enc->setBuffer(p, 0, 0);
                enc->setBuffer(r, 0, 1);
                enc->setBuffer(v, 0, 2); 
                ...
                //spVmV:  sigma (scalars[4]) = r0 A p = r0 v
                ...
                enc->setBuffer(this->vals, 0, 2);
                enc->setBuffer(p, 0, 3);
                enc->setBuffer(r0, 0, 4);
                enc->setBuffer(v, 0, 5);
                enc->setBuffer(scalars, 4 * sizeof(float), 6);
                enc->setBytes(&this->num_block_rows, sizeof(uint32_t), 7);
                enc->dispatchThreads(MTL::Size::Make(this->num_block_rows, 1, 1), MTL::Size::Make(THREADS_PER_GROUP, 1, 1));
                enc->memoryBarrier(MTL::BarrierScopeBuffers);

                //upalpha_ups: alpha (scalars[2]) = rho_curr / sigma , s = r - alpha * v 

                enc->setComputePipelineState(this->pso_upalpha_ups);
                enc->setBuffer(s, 0, 0);
                enc->setBuffer(r, 0, 1);
                enc->setBuffer(v, 0, 2);
                enc->setBuffer(scalars, 0, 3);
                ...
                mv_buffer(s, t, cmd, enc);
                enc->memoryBarrier(MTL::BarrierScopeBuffers);

                //f_dip = phi_1 = t * s, phi_2 = t * t
                enc->setComputePipelineState(this->pso_dip);
                enc->setBuffer(s, 0, 0);
                enc->setBuffer(t, 0, 1);
                enc->setBuffer(scalars, 0, 2);
                enc->setBytes(&this->num_rows, sizeof(uint32_t), 3);
                enc->dispatchThreads(MTL::Size::Make(this->num_rows, 1, 1), MTL::Size::Make(THREADS_PER_GROUP, 1, 1));
                enc->memoryBarrier(MTL::BarrierScopeBuffers);

                //f_upomega_upxr_laa

                enc->setComputePipelineState(pso_upomega_upxr_laa);
                enc->setBuffer(x, 0, 0);
                enc->setBuffer(r, 0, 1);
                enc->setBuffer(p, 0, 2);
                enc->setBuffer(r0, 0, 3);
                enc->setBuffer(s, 0, 4); 
                enc->setBuffer(t, 0, 5);
                ...
                if (i % CHECK_ITERATION_NUMBER == 0) {
                    if (i > 0) {
                        if (stage_r_norm_ptr[0] < TOLERANCE) {
                            left_early = true;
                            break; 
                        }
                    }
                    ...
                    MTL::BlitCommandEncoder* blit_check = cmd->blitCommandEncoder();
                    blit_check->copyFromBuffer(scalars, 7 * sizeof(float), stage_r_norm, 0, sizeof(float));
                    blit_check->endEncoding();
            ...
            memcpy(x_vals, stage_x->contents(), (this->logical_cols) * sizeof(float));
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    py::class_<bcsr_tensor>(m, "bcsr_tensor")
        .def(py::init<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, int, int, int>(),
             "BCSR Tensor Constructor",
             py::arg("keys"),
             py::arg("values"),
             py::arg("row_ptr"),
             py::arg("col_ind"),
             ...
             py::arg("num_rows"),
             py::arg("num_cols"))
        .def("cgd", &bcsr_tensor::cgd, "cgd")
        .def("bicgstab", &bcsr_tensor::bicgstab, "bicgstab");
}